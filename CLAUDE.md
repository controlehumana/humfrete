...
**Fusão Vladimir/Henrique Dezembro (jul/2026):** usuário confirmou que VLADIMIR ROBERTO DEZEMBRO (CNPJ 33.585.276/0001-38) e HENRIQUE PACETTI DEZEMBRO (CNPJ 30.184.082/0001-32) são a mesma operação de entrega faturando por 2 CNPJs diferentes (ambos atendem BRU1+BRU2) — na prática, quando um não emite NFS-e no mês é porque o outro emitiu. Olhar os 2 separadamente na matriz gerava falso alerta (✕) num mês em que a operação como um todo tinha faturado, só pela outra CNPJ. **Fix (só na matriz `dlv_gap_matrix`, não no ranking):** `GAP_ALIAS` dentro da IIFE que monta a matriz mapeia os dois nomes pra um `'VLADIMIR / HENRIQUE DEZEMBRO'` combinado antes de agregar por `entregador_nome`; o merge `temNfse=temNfse||v.tem_nfse` já existente faz o OR automaticamente (✓ se qualquer um dos dois faturou naquele mês). Testado: dos ~23 meses, só 2 continuam ✕ mesmo combinados — esses são gap real. Ranking (`dlv_rank_tbody`) continua mostrando os 2 separados (não pedido, mudança mais visível de valores históricos, não implementado sem confirmação explícita).

**MURALHA ENTREGA RAPIDA LTDA (CNPJ 48.361.800/0001-64, CGR) — cadastrada jul/2026, zero NFS-e:** usuário identificou que é um serviço "tipo Uber de entregas" em Campo Grande — nunca vai ter CT-e (mesmo padrão de entregador autônomo). Cadastrada em `entregadores.xlsx`, mas **também não tem nenhuma NFS-e emitida pra nós** (testado via `buscar_nfse_entregadores.py`) — provavelmente é cobrada via plataforma/app, não fatura direto pra Humana. Por isso não vai aparecer com nenhum ✓ na matriz de Controle de Nota de Serviço (não há dado de nota nenhuma, nem certo nem errado) — não é bug, é a realidade desse modelo de cobrança. Se o modelo de cobrança mudar no futuro, o cadastro já está pronto pra capturar.
