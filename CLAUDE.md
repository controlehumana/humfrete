...
**Módulo "Devolução Compra" (aba própria, 2026-09-24):** usuário pediu explicitamente um módulo separado (não só filtro dentro de Frete Compras) pra visualizar esse custo isolado. Implementado espelhando 1:1 a estrutura de Devolução Venda: `_carregar_nf_devolucao_compra()` (SQL direto `cte_nf` join `nf_devolucao_venda WHERE status='compra'`, sem view nova no QUIVE), lista `devolucao_compra` diverida nos mesmos 2 pontos do `eh_devolucao`/loop de CTe destinatário Humana (branch `elif _cat_nf == "compra"`), payload/`split_by_empresa`/`_mergeData` com a chave `devolucao_compra`. Frontend: aba `devolucao-compra`, IDs `devc_*`, reaproveita 100% o padrão visual/funcional de `devv_*` (mesmas colunas, botão Ver DANFE, export Excel). **Não integrado ao DRE Logístico** de propósito (fora do escopo pedido, e mudar a cascata do DRE exige cuidado extra de dupla-contagem) — se um dia quiser, adicionar linha "Frete de Devolução de Compra" em `montar_dre()` seguindo o mesmo padrão de `devolucao_venda`.
**Contagem final publicada: 40 CT-e** (dos 86 identificados pela query SQL) — a diferença são CT-e que também carregam NF-e de venda normal no mesmo documento; esses ficam em `detalhes`/Frete de Saída por `linked_cte_chaves` já os ter capturado antes, evitando dupla contagem (mesmo mecanismo de dedup que já existia pra Devolução Venda).
...
**A matriz de controle diz isso na cara (ago/2026):** o card `dlv_gap_card` sempre organizou as colunas por competência, mas nada na tela dizia isso — o usuário pediu que ficasse visível. Ganhou uma linha fixa abaixo do título com o exemplo concreto ("emitida em 03/08 com competência 31/07 conta em Jul"), o cabeçalho da coluna congelada virou "Entregador / competência →", cada `<th>` de mês tem `title="Competencia Mmm/AAAA"` e a legenda passou de "NFS-e emitida no mes" para "NFS-e com competencia nesse mes". **Regra do projeto:** quando um painel escolhe uma entre duas datas plausíveis, dizer qual é na própria tela — tooltip não basta, ninguém abre antes de tirar conclusão.
