...
- `_transfRenderLevantamento(lvt)` — tabela `transf_lvt_tbody`, 1 linha por unidade/mês, guarda o array em `_transfLvtRows` para o export. Também preenche `<tfoot id="transf_lvt_tfoot">` com a soma de cada coluna numérica (`geral,delivery,m2u,u2m,entre`) sobre o `lvt` já filtrado — linha "Total" ao final da tabela, fundo `var(--s3)` (tom levemente mais escuro que as linhas normais) aplicado **em cada `<td>`**, não no `<tr>`
