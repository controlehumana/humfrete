...
### `_transfRenderMatrix(rows)` — Matriz de Frete Unidade Origem x Destino (jul/2026)
Visão alternativa do Levantamento Geral pedida pelo usuário — a tabela linear "não estava clara"; a matriz cruza **unidade origem (linha) × unidade destino (coluna)**, sem quebra por mês (soma o período filtrado inteiro). Adicionada **ao lado** do Levantamento Geral, não substituiu (decisão explícita do usuário).
- `Venda` e `Delivery` entram como **colunas fixas** (não têm "unidade destino" — vão para cliente externo ou entregador autônomo) — decisão explícita do usuário, alternativa seria deixá-las fora da matriz
- Para cada linha de `rows`, se `_transfDestino(d)` for truthy, soma em `byOrigem[d.empresa].destinos[destino]`; senão soma em `.venda` — **não usa `_transfCategoria(d)`**: a direção (Matriz→Unidade/Unidade→Matriz/Entre Unidades) fica implícita pela posição linha×coluna (ex.: linha BRU1/coluna CMP É "Matriz→Unidade" para CMP, sem precisar de rótulo) — mais simples que a lógica de categoria usada em outras partes da aba
- Célula da diagonal (origem===destino, ex. BRU1/BRU1) recebe `background:var(--input-bg)` só para destacar visualmente — é um dado real (ver pitfall de auto-transferência abaixo), não filtrado/zerado
- Coluna `Total` = `venda+delivery+soma(destinos)` — **conferido igual a `SUM(lvt.filter(r=>r.emp===X).geral)`** (a soma de "Fretes Gerais" de todos os meses daquela unidade no Levantamento Geral) — validado com dados reais (BRU1: R$711.822,44 nos dois lugares)
- Container `#transf_matrix_wrap` (não é `<table>` fixo no HTML — o `_transfRenderMatrix` monta a tabela inteira via `innerHTML`, mesmo padrão de `renderYoY()`/`_transfRenderDetalhe` gap matrix), sticky-left na coluna "Unidade Origem" (mesmo `stickyBase`/`thBase` dos outros heatmaps)
- Sem paginação/export — é uma visão de overview, não substitui o Excel do Levantamento Geral
...
- **Card `dlv_gap_card`** ("Controle de Nota de Serviço por Entregador") — matriz entregador (linha, sticky à esquerda) × mês (coluna), mesmo padrão visual do heatmap `renderYoY()` (`stickyBase`/`thBase`, `var(--sticky-bg)`), mas com 3 estados discretos em vez de gradiente: ✓ verde (`var(--green2)`) = tem NFS-e autorizada naquele mês; ✕ vermelho (`var(--red2)`) = entregou mas não tem NFS-e; "—" cinza = sem entrega registrada naquele mês (não é gap, é normal). Tooltip (`title` nativo) mostra qtd de NF-e e valor de mercadoria ao passar o mouse. Substituiu uma primeira versão em lista (só linhas sem NFS-e) — usuário achou a lista pouco clara pra entender o padrão mês a mês; a matriz mostra o histórico completo de cada entregador de uma vez
...
- **CT-e de "auto-transferência" (origem === destino) existe nos dados reais** — ex.: 54 CT-e de BRU1 com `_transfDestino(d)==='BRU1'` no período testado (jul/2026). Não é bug do parsing — o CT-e real tem CNPJ destinatário igual ao remetente (provável erro de emissão do lado da transportadora/ERP, ou uma modalidade de transporte que não é transferência de fato mas cai na mesma classificação por coincidência de CNPJ). `_transfCategoria` classifica como `'m2u'` sempre que `d.empresa===MATRIZ_UNIDADE`, independente do destino — não filtra esse caso. Aparece na diagonal da Matriz de Frete (`_transfRenderMatrix`) com destaque visual (`background:var(--input-bg)`), mas o valor É somado no Total normalmente. Se o volume desses casos crescer a ponto de distorcer os números, investigar a causa raiz no CT-e antes de simplesmente excluir do cálculo.
